...
> 版本：v3 · 2026-09-05 · Roger Cui（itabas016）× Claude Code
...
v2 方案（跑马灯 → 精选大卡 + 筛选网格）解决了结构问题，但评审发现精选区引入了新的 AI 味装饰（渐变横幅、水印大字、emoji pill）。v3 在结构不变的前提下做减法。
...
- ✦ vibe-coded 徽章：全站 AI 披露母题，与 `/ai` 页呼应。

## 4. 页面结构

```
Hero：H1 + 一句话说明 + 数字条（应用 11 · 在线 8/9 · vibe-coded 9/9，tabular-nums）
──────────────────────────────────────────
Featured：3 行排印索引（分类 · 日期｜大字应用名｜一句话｜stack chips｜✦ vibe-coded · 打开应用 ↗）
         cover 字段就绪后，行右侧接 16:10 截图缩略图
──────────────────────────────────────────
筛选 pills：全部 9 ｜ 实验 1 ｜ 数据 2 ｜ 阅读 2 ｜ 生活 3 ｜ 工具 1（mono 计数）
应用网格：3 列 → 2 列 → 1 列，既有 AppCard；wip 徽章；默认视图不含精选应用
──────────────────────────────────────────
早期作品时间线：一行一条（年月 · 名称 ↗ · 一句话），灰调，2017 → 2020
```
...
- **筛选**：约 40 行 vanilla JS 渐进增强。筛选态同步 `?c=<id>` 到 URL（可分享、可后退）；pill 用 `aria-pressed` 驱动选中态，`focus-visible` 有 2px ring。
- **精选让位规则**：「全部」时精选应用只在索引区出现（网格里服务端渲染为 `hidden`，无 JS 也不重复）；筛选时索引区隐藏，被筛中的精选应用回到网格，与 pill 计数一致。
- **无 JS 降级**：筛选条隐藏，精选索引 + 全量网格照常可读，零内容丢失。
- **重排动效**：淡入上移 200ms（`cubic-bezier(0.23,1,0.32,1)`），stagger 40ms；`prefers-reduced-motion` 时关闭。
- **循环动画**：本页 0 个（跑马灯已移除）。
...
筛选态（生活，URL `?c=life`，精选区让位、BadmintonSettle 回网格）：

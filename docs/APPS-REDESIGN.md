...
> 版本：v4.1 · 2026-09-05 · Roger Cui（itabas016）× Claude Code
...
| 2 | Featured 形态 | 3 行排印索引（手工 `featured` 挑选） | **全量应用大卡自动轮播**：一屏 3 卡（lg）/ 2 卡（sm）/ 1 卡（移动），5s 自动步进 1 卡并循环；两侧 ‹ › 箭头手动按页翻（一次 perView 张），页码点可直达 | 推广位要给每个应用出场机会，一屏多卡减少翻页成本，手动翻页不必等自动滚动 |
...
- **轮播**：scroll-snap 横向轨道 + JS 自动翻页（5s 步进 1 卡，到尾循环）。两侧 ‹ › 箭头按页翻（一次一屏的卡片数：桌面 3 / 平板 2 / 移动 1），页码点直达任意卡；悬停/聚焦暂停，离开恢复；`visibilitychange` 时停走；`prefers-reduced-motion` 停用自动播放且翻页不动画。无 JS 时退化为原生横滑（snap 兜底），箭头与页码点隐藏。
